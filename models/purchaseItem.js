...
exports.post = function (req, res, next) {
	...
	pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		client.query('insert into purchaseItem (id, idShoppingCart, idProduct, status, amountRequested, amountPurchased) values ($1,$2,$3,$4,$5,$6) RETURNING _id, id, idShoppingCart, idProduct, status, amountRequested, amountPurchased',[req.body.id,req.body.idShoppingCart,req.body.idProduct,req.body.status,req.body.amountRequested,req.body.amountPurchased], function(err, result) {
			done();
			if (err) {
				console.error(err);
				response.send("Error " + err);
   ...
   pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		client.query('SELECT * FROM purchaseItem where id = $1',[req.body.id], function(err, result) {
			...
			if (err) {
				...
				var userFound = result.rows;
				client.query('update purchaseItem set idShoppingCart=$2, idProduct=$3, status=$4, amountRequested=$5, amountPurchased=$6 where id = $1',[req.body.id,req.body.idShoppingCart,req.body.idProduct,req.body.status,req.body.amountRequested,req.body.amountPurchased], function(err, result) {
					done();
					if (err) {
						console.error(err);
						response.send("Error " + err);
