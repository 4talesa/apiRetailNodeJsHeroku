...
exports.post = function (req, res, next) {
	...
	pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		client.query('insert into shoppingCart (id, idStore, idUser, status, idPaymentMethod, date, postDate, putDate, deleteDate) values ($1,$2,$3,$4,$5,now(),now(),null,null) RETURNING _id, id, idStore, idUser, status, idPaymentMethod, date, postDate, putDate, deleteDate',[req.body.id,req.body.idStore,req.body.idUser,req.body.status,req.body.idPaymentMethod], function(err, result) {
			done();
			if (err) {
				console.error(err);
				res.send("Error " + err);
		...
		client.query(default_select+' where p.idUser = $1 and p.idStore = $2',[req.params.idUser,req.params.idStore], function(err, result) {
			...
			if (err) {
				...
				if (result.rows.length == 0){
					...
					client.query('insert into shoppingCart (id, idStore, idUser, status, idPaymentMethod, date, postDate, putDate, deleteDate) values ($1,$2,$3,$4,$5,now(),now(),null,null) RETURNING _id, id, idStore, idUser, status, idPaymentMethod, date, postDate, putDate, deleteDate',[req.params.idUser+'.'+req.params.idStore,req.params.idStore,req.params.idUser,'pending','1'], function(err, result) {
						done();
						if (err) {
							console.error(err);
							res.send("Error " + err);
